# ADL LSI BI FTP Decoder

ADL Plugin for collecting data from Burundi LSI FTP
...
The ADL LSI BI FTP Decoder is a module intended to be installed in an [ADL](https://github.com/wmo-raf/adl)
...
### Install ADL LSI BI FTP Decoder

The `dev.Dockerfile` file uses the `adl` image as a base image. The `ADL LSI BI FTP Decoder` is
...
```bash
git clone https://github.com/wmo-raf/adl-lsi-bi-ftp-decoder.git
cd adl-lsi-bi-ftp-decoder
```
